...
类还可能有静态成员，他们并不会与特定实例相关联。他们可以通过类构造函数对象本身访问:
...
静态成员同样可以使用 public、ptotected 和 private 可见性修饰符:
...
从函数原型对象上重写属通常来说是不安全/不可能的。因为类本身就是可以用 new 调用的函数，所以不能使用某些静态名称，像 name、length 和 call 这样的函数属性，是不能被定义为静态成员的:
...
ts 并没有像 C# 那样的名为 `static class` 的结构。

这种结构之所以存在，是因为那些语言强制要求数据和函数必须在一个类中，由于 ts 中并没有这种限制，所以不需要它们。只有一个实例的类在 js/ts 中通常被表示为一个普通的对象。

举个例子，在 ts 中我们并不需要一个 `静态类` 语法，因为一个常规的对象(甚至顶级作用域函数)也可以完成这项工作。
...
静态代码块允许你编写一些列的具有自己作用域的语句，这些语句可以访问到包含类的 private 字段。这意味着我们可以编写具有声明语句的初始化代码，而无需担心变量泄漏，并能完全访问类的内部结构。
...
记住 ts 并不会修改 js 运行时的行为是很重要的，尤其 js 以具有一些运行时怪癖而闻名。
...
这应该是你不想看到的事情! ts 提供了一些方法来减少或阻止这种错误。
...
- `this` 的在运行时保证是正确的，即使对于未经 ts 检查过的代码
- 这将使用更多的内存，因为每个类实例都有自己的以这种方式定义的函数副本
- 在派生类中你无法使用 `super.getName`，因为在原型链中无法获取基类方法

### this 参数

在一个方法或函数定义中，在 ts 中一个叫 `this` 的初始参数具有特殊的意义,它在编译期被擦除:
...
ts 检查调用一个具有 this 参数的函数是否在正确的上下文被调用，我们可以将 this 参数添加到方法定义中，而不是使用箭头函数，以静态的方式强制方法被正确调用了:
...
const b = a.set('hello'); // b: ClearBox
```
...
你可以在类方法和接口的返回值标注位置使用 `this is Type`。当与类型收窄混合使用，目标对象的类型将被收窄为指定的 `Type`
...
ts 提供了一种特殊的语法，可以将构造函数参数转为具有相同名称和值的类属性。这些称为 *参数属性*，通过在构造函数参数前添加可见性修饰符 public、protected、private 和 readonly 之一来创建，由此产生的字段也会获取到这些修饰符
...
类表达式和类声明是类似的，它们之前唯一真正的区别在于类表达式不需要一个名称，虽然我们可以通过它绑定的变量来引用它：
...
有时你想接受一些类的构造函数，它产生一个派生自某个抽象类的类实例。
...
ts 正确的告诉你，你在尝试初始化一个抽象类。毕竟，鉴于 greet 的定义，写出这种代码完全是合理的，但它最终会导致构造一个抽象类：
...
空类没有成员，在一个结构化类型系统中，一个没有成员的类型通常是其他任何类型的超类。所以如果你写了一个空类(不要这么做)，任何都可以用来替代它:
